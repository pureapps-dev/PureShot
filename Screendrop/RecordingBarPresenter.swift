...
private struct RecordingBarView: View {
    ...
    private var visibilityBanner: some View {
        HStack(spacing: 8) {
            ...
            Text("PureShot windows will be captured")
                .foregroundStyle(BarMetrics.activeTint)
            Spacer(minLength: 8)
            Toggle("Include PureShot windows in captures", isOn: $includeAppWindowsInCaptures)
                .labelsHidden()
                .toggleStyle(.switch)
                .controlSize(.mini)
                .accessibilityHint("Turn off to hide PureShot windows from screenshots and recordings")
        }
